...
- **FR-14** 임계값 미만 공지는 저장은 하되('전체 공지' 열람용) 피드의 'AI 추천'·알림 대상에서는 제외한다. `[as-built] is_recommended` 플래그로 구분.
...
- **FR-20** `is_recommended=true` 공지가 미발송이면 선택한 채널로 발송한다. `[as-built]` 발송 시기: (a) 채널 연동 시 확인 메시지, (b) 사이트 동기화 후 신규 추천분(비차단, best-effort), (c) 1시간 주기 스케줄러.

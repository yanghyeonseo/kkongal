...
- **중복 방지·재시도**는 `InboxNotice.notified_at` 으로 한다. **한 채널이라도 성공하면** `notified_at` 을 갱신한다(성공 채널로의 중복 발송 방지, 실패 채널은 재시도하지 않고 `AlertLog` 에 사유만 남긴다). 반면 **전 채널이 실패**(일시적 SMTP/webhook 장애 등)하면 `notified_at` 을 남겨 다음 주기에 재시도한다 — 일시 장애로 인한 알림 유실을 막는다.

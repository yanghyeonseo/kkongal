...
LLM 클라이언트는 **제공자 비종속(OpenAI 호환)** 이며 **3단 캐스케이드**로 무료 티어 한도를 방어한다: **1차 = `LLM_MODEL`(primary)** → **2차 = `LLM_FALLBACK_MODEL`(같은 `LLM_BASE_URL`·`LLM_API_KEY`, 모델 문자열만 다름)** → **3차 = 결정론적 키워드/설명 매칭 폴백**. 1·2차 중 하나라도 성공하면 정상(`mark_ok`)이고, **두 LLM 계층이 모두 실패할 때만** 키워드 폴백으로 내려가며 배너 플래그(`mark_degraded('quota')`)를 남긴다. 모든 LLM HTTP 요청은 **프로세스 전역 스로틀**(`LLM_MIN_REQUEST_INTERVAL_SECONDS`, 기본 4.5초)로 최소 간격을 둬 분당 요청수(RPM)를 낮춘다. `LLM_BASE_URL`/`LLM_MODEL`/`LLM_API_KEY` 세 값만 바꾸면 OpenAI·DeepSeek·Groq 등 어떤 OpenAI 호환 제공자로도 교체되고(제공자 교체 = base_url·model·key 변경), **키가 없거나 모든 LLM 계층이 실패해도 예외를 던지지 않고 결정론적 키워드 매칭으로 폴백**하므로 오프라인/CI/데모에서도 파이프라인이 그대로 돈다. 이 앱은 HTTP 왕복 없이 Django ORM 을 직접 쓰며(관리 명령·서비스 함수), 유일한 HTTP 노출은 프론트 배너용 상태 조회다.

## 구성

| 파일 | 역할 |
| --- | --- |
| `llm.py` | `LLMClient`(OpenAI 호환 chat/completions, 3단 모델 캐스케이드, 전역 요청 스로틀, 503 재시도, 방어적 JSON 파싱, 키워드 폴백), `Verdict`·`Enrichment` 결과 타입, `first_sentences`(폴백 요약), `get_client` 팩토리 |
